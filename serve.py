#!/usr/bin/env python3
"""Verb Practice — local web interface.

Run:  python serve.py
Starts a small local server (standard library only, nothing leaves your
computer except the optional Gemini / TTS requests) and opens the practice
interface in your browser.  The classic desktop interface is still available
with  python main.py .
"""
...
def fetch_phrases_sync(needed, key, timeout=45):
    """Run the cache's async fetch and wait for it (one at a time)."""
...
def build_block(cat, mode, forms, words):
    """Practice payload for one block: answers, sentences, meanings."""
    ...
    for v in words:
        ...
        for c in cols:
            ...
            fields.append({
                "col": c, "answer": ans, "sentence": sentence,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _state(self):
        ...
        return {"cats": cats,
                "voice": store.voice(), "voices": VOICES,
                "theme": store.prog.get("theme", "light"),
                "themes": list(THEMES),
    ...
    def _audio(self, word):
        ...
        self.send_header("Content-Length", str(len(data)))
        self.send_header("Cache-Control", "no-store")   # voice can change
        self.end_headers(); self.wfile.write(data)
    ...
    def _session(self, b):
        ...
        blocks = [b_ for b_ in store.enabled_blocks(cat) if b_]
        sizes = [len(x) for x in blocks]
        total = sum(sizes)
        comp = min(store.comp(cat), total)
        # continue at the last block whose start offset is <= completed
...
def main():
    ...
    print(f"Verb Practice web  →  {url}   (Ctrl+C to stop)")
